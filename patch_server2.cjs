...
let contentIngestion = fs.readFileSync('src/server/steIngestionService.ts', 'utf-8');
contentIngestion = contentIngestion.replace(/export async function reindexSteDocument[\s\S]*$/, `export async function reindexSteDocument(id: string): Promise<void> {
...
fs.writeFileSync('src/server/steIngestionService.ts', contentIngestion);
