...
export default function SummaryWorkspace({ workspace, onChange, acceptedNeighbors }: Props) {
  ...
  const handleRewrite = async () => {
    ...
    onChange(prev => beginRequest(prev, requestId));
    try {
      const { result, warnings: newWarnings } = await rewriteSummary(workspace.raw, content, instructions, acceptedNeighbors);
      onChange(prev => {
        if (!isResponseCurrent(prev, requestId)) return prev;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-4 bg-amber-50 border border-amber-100 rounded-lg space-y-3">
          ...
          <Collapsible label={`Add more detail (${OPTIONAL_FIELDS.length} optional fields)`} defaultOpen={optionalFieldsFilled}>
