...
import { SummaryAccepted, SectionWorkspace } from "./announcementTypes";
import {
  editRaw,
  editComponents,
  beginRequest,
...
const EMPTY: SummaryAccepted = { centralMessage: "" };
const CORE_FIELDS: (keyof SummaryAccepted)[] = ["centralMessage", "affectedScope", "impact"];
const OPTIONAL_FIELDS: (keyof SummaryAccepted)[] = ["implementationTiming"];
...
const LABELS: Record<string, string> = {
  ...
  impact: "Impact",
  implementationTiming: "Implementation Timing",
};
const GUIDANCE: Record<string, { placeholder?: string; hint?: string }> = {
  centralMessage: { placeholder: "What is being announced -- the change, update, requirement, or concern.", hint: "The heart of the Announcement." },
  affectedScope: { placeholder: "What/who is affected (products, equipment, locations, teams).", hint: "Optional -- leave blank if the source doesn't say." },
  impact: { placeholder: "Why it matters -- the effect on users or operations.", hint: "Optional." },
  implementationTiming: { placeholder: "When this takes effect, or a stated deadline.", hint: "Optional -- only if the source states one; never invented." },
};

// Local per-section operation status, distinct from workspace.loading (which
// drives disabling/dedup). Powers richer text and a success confirmation that
// never says "Accepted" -- a check here must not imply canonical status.
type OpStatus =
  | { kind: "idle" }
...
function SummaryContent({ value }: { value: SummaryAccepted }) {
...
export default function SummaryWorkspace({ workspace, onChange }: Props) {
  ...
  // Mirror of the latest workspace, so async completion handlers can check
  // response currency (isResponseCurrent) WITHOUT calling setOpStatus/side
  // effects inside the setDraft updater -- doing that triggers React's "cannot
  // update a component while rendering a different component" warning. The
  // updater stays pure; opStatus and scrolling happen out here.
  ...
  const setField = (field: keyof SummaryAccepted, value: string) =>
