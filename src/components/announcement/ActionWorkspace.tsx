...
import { Collapsible, StaleExplanation, FieldHint } from "./AnnouncementHelpers";
import { ACTION_KIND_LABELS, actionItemMeta } from "./announcementActionRender";
import { actionHasContent } from "./announcementReadiness";
...
function ActionItemsView({ value }: { value: ActionAccepted }) {
  return (
    <div className="space-y-1.5 text-sm">
      {value.lead && value.lead.trim() && <RichTextContent text={value.lead.trim()} className="space-y-1" />}
...
export default function ActionWorkspace({ workspace, onChange }: Props) {
  ...
  const handleRewrite = async () => {
    ...
    onChange((prev) => beginRequest(prev, requestId));
    try {
      const { result, warnings: newWarnings } = await rewriteAction(workspace.raw, content);
  ...
  const hasRequiredContent = actionHasContent(content);
        ...
        <>
          ...
          {showComponents && (
            <div className="p-4 bg-teal-50 border border-teal-100 rounded-lg space-y-3">
              ...
              <FieldHint text="Item text supports **bold**, *italic*, [link](url), and &quot;- &quot; for sub-steps." />
              <div>
