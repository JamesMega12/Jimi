...
function ReasonContent({ value }: { value: ReasonAccepted }) {
  ...
  return (
    <div className="space-y-2">
      {value.renderedText ? (
        value.renderedText.split(/\n{2,}/).map((p, i) => <p key={i} className="text-sm leading-relaxed">{p.trim()}</p>)
      ) : (
        <div className="text-sm space-y-1">
          {value.rationale && <p>{value.rationale}</p>}
          {value.triggeringObservation && <div><strong>Triggering Observation:</strong> {value.triggeringObservation}</div>}
        </div>
      )}
      {cause && (
...
export default function ReasonWorkspace({ workspace, onChange }: Props) {
  ...
  const handleRewrite = async () => {
    ...
    onChange((prev) => beginRequest(prev, requestId));
    try {
      const { result, warnings: newWarnings } = await rewriteReason(workspace.raw, content, instructions, causeStatusEdited);
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 bg-teal-50 border border-teal-100 rounded-lg space-y-3">
          <h4 className="font-bold text-slate-800 flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5 text-emerald-600" /> Reason Components
          </h4>
          <div>
